...
#打开要搜索论文的txt文本
def get_paper_names():
    fo = open('names.txt','r',encoding='UTF-8') #打开names文件
    names=[]
    for line in fo:
        line = line.replace('\n','')
        names.append(line)
...
def get_paper_nums(browser,url,num=1):#num：爬取单个论文名字的编号个数
    ...
    try:
        WebDriverWait(browser,5,0.5).until(lambda x: x.find_element_by_xpath("//*[@data-artnum]"))
    except:
        pass
    ele_nums = []
    try:
...
def get_download_links(ele_nums):
    ...
    i=1
    for ele_num in ele_nums:
        src = 'https://ieeexplore.ieee.org/stampPDF/getPDF.jsp?tp=&arnumber='+ele_num+'&ref='
        srcs.append(src)
...
def DownOneFile(srcUrl, localFile):
    ...
    with requests.get(srcUrl,stream=True,headers=headers) as r:# timeout=80,stream=True
...
def DownFiles(srcUrls,titles):
    ...
    for i in range(len(srcUrls)):
        ...
        downSize=DownOneFile(srcUrls[i], filename)
        # while((downSize/1024/1024)<=0.05):
        #     downSize = DownOneFile(srcUrls[i], filename)
    print("\n" + "执行结束".center(50 // 2, '-'))
...
if __name__ == '__main__':
    ...
    for i in range(len(urls)):#获取要下载论文的编号
        # 此处设置获取每个论文名字下的论文个数，可根据实际修改个数，默认为1，这里是3。
    ...
    srcs = get_download_links(ele_nums)
